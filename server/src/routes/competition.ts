...
router.put('/save', isAdmin, (req, res) => {
    ...
    let pageUploads = uploadFiles(data.pages, uploaddir, page => Boolean(page.html), _ => '_temp',
        page => `/pages${page.path.length == 1 ? '' : page.path}/_page`, page => parentDir(page.source),
    ...
    let sponsorUploads = uploadFiles(data.sponsors, uploaddir, sponsor => Boolean(sponsor.img), sponsor => '_' + sponsor.name,
        sponsor => `/sponsors/${sponsor.name}`, sponsor => parentDir(sponsor.icon), (sponsor, dir) => upload(dir, sponsor.img),
    ...
    Promise.all([pageUploads, sponsorUploads]).then(() => Promise.all([
        ...
        DB.setRepo(DB.repo(Page), data.pages.map(x => new Page(x)), undefined, x => [parentDir(uploaddir + x.source)]),
        DB.setRepo(DB.repo(Sponsor), data.sponsors.map(x => new Sponsor(x)), undefined, x => [parentDir(uploaddir + x.icon)])
